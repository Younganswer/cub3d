...
static t_bool	set_texture_file_path(t_texture *walls, int fd);
...
t_bool	parse_texture(t_game *game, int fd)
{
	...
	return (set_texture_file_path(game->space->walls, fd) && \
			xpm_to_img(game)
	);
}
...
static t_bool	set_texture_file_path(t_texture *walls, int fd)
{
	...
	while (i++ < 4)
	{
		...
		free(line);
		if (idx < 0 || check_file_extension(walls[idx].file, ".xpm") == FALSE)
			return (FALSE);
	}
	return (TRUE);
}
