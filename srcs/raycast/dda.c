...
static t_bool	set_dda_var(t_game *game, int col)
{
	...
	raycast->step.x = 1;
	raycast->side_dist.x = (raycast->map.x + 1.0
			- game->player->pos.x) * raycast->delta_dist.x;
	if (raycast->ray_dir.x < 0)
	{
		raycast->step.x = -1;
		raycast->side_dist.x = (game->player->pos.x
				- raycast->map.x) * raycast->delta_dist.x;
	}
	raycast->step.y = 1;
	raycast->side_dist.y = (raycast->map.y + 1.0
			- game->player->pos.y) * raycast->delta_dist.y;
	if (raycast->ray_dir.y < 0)
	{
		raycast->step.y = -1;
		raycast->side_dist.y = (game->player->pos.y
				- raycast->map.y) * raycast->delta_dist.y;
	}
	return (TRUE);
}
